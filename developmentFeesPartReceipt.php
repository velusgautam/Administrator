...
<head>
	<?php
		...
		$partId = $_GET['partId'];
		if (intval($partId) > 0) {
			$sql = "SELECT TSDP.payment_now, TSDP.balance, TSDF.id, TSDF.id, TSDF.`student_name`, TSDF.`academic_year`,TSDF.`date`,TSDF.development_fees, TSDF.waive_off, TSDF.add_on, TSDF.total,  TSDF.payment_type, TSDF.payment_status,
			TSDF.cheque_no,
			DATE_FORMAT(TSDF.cheque_date,
		'%d-%m-%Y') as cheque_date ,
		TSDF.cheque_bank ,
		TC.`class_name`, TS.stream_id,
		TD.`division_name` FROM " . TABLE_STUDENT_DEVELOPMENT_FEE_PART . " TSDP
		        ...
		        WHERE TSDF.id = " . $id." ORDER BY TSDP.part_id DESC";
			$rows = $db->query_first($sql);
		} else {
			$sql = "SELECT TSDF.id, TSDF.`student_name`, TSDF.`academic_year`,TSDF.`date`,TSDF.development_fees, TSDF.waive_off, TSDF.add_on, TSDF.total,  TSDF.payment_type, TSDF.cheque_no, DATE_FORMAT(TSDF.cheque_date,
		'%d-%m-%Y') as cheque_date ,
		TSDF.cheque_bank ,
		TC.`class_name`, TS.stream_id,
		TD.`division_name` FROM " . TABLE_STUDENT_DEVELOPMENT_FEE . " TSDF
		        INNER JOIN " . TABLE_STUDENT . " TS ON TS.student_id = TSDF.student_id
		        INNER JOIN " . TABLE_CLASS . " TC ON TS.class_id = TC.class_id
		        INNER JOIN " . TABLE_DIVISION . " TD ON TS.division_id = TD.division_id
		        WHERE TSDF.id = " . $id;
								...
								<tr>
									<td colspan="3" style="line-height: 2">
										<?php if ($rows['payment_status'] == 0) {
											...
											<?php
										} elseif ($rows['payment_status'] == 1) {
											?>
											Received with thanks from A sum of <strong><?php echo rtrim(trim(ucwords(notowords(intval($rows['payment_now'])))), "&") . " " ?></strong>Rupees
											Only towards the Annual Development Fees for the Academic Year:  <?php echo $rows['academic_year']?>.
