...
// Global configuration /////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*
	* The directory is relative to "wixzify"-folder and should be write-enabled
...
// CSS specific configuration ///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*
	* When being in string-mode, the wixzify prepends this path going out from the caller-location
...
// Start of functions ///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*/
	private function errorcheck()
	{
		...
		{
			$this->errormessage = "\r\n".'$_SERVER[\'DOCUMENT_ROOT\'] variable is set to '.$_SERVER['DOCUMENT_ROOT'].'. But you seem to have a differing real document root. Please set the variable $wixzify->document_root to reflect your real document root'."\r\n";
		}
		...
		{
			$this->errormessage = "\r\n".'$wixzify->base_offset variable is set to '.$this->base_offset.'. But this server\'s document root seems to be differently offsetted. Please set the variable $wixzify->base_offset to reflect this offset'."\r\n".$this->base_offset.ltrim(str_replace($this->document_root,'',$_SERVER['SCRIPT_FILENAME']),'/')."\r\n".parse_url($_SERVER['SCRIPT_NAME'],PHP_URL_PATH)."\r\n";
		}
	}
	...
	*/
	...
	{
		// Calculate absolute path for wixzify-folder
	...
	public function css()
	{
		...
		else
		{
			...
			if(is_dir($this->booster_cachedir))
			{
				...
				{
					// If it is a file and the filetype matches and it isn't the log file 
					// and last file modified time is older than wixzify library itself, then delete
					if($file[0] != '.' && 
						strtolower(pathinfo($this->booster_cachedir.'/'.$file,PATHINFO_EXTENSION)) == 'txt' && 
						$this->booster_cachedir.'/'.$file != $this->debug_log && 
	...
	*/
	public function css_markup()
	{
		...
		// Put together the markup linking to our wixzify-css-files
		...
		if($this->css_lazyload) $markup .= '<script type="text/javascript">//<![CDATA['."\r\n".'var booster_stylesheets = new Array('.implode(',',$linkarray).');'.file_get_contents(str_replace('\\','/',dirname(__FILE__)).'/wixzify.css_lazyload.min.js')."\r\n".'//]]>'."\r\n".'</script>'."\r\n";
		$markup .= '<!--[if IE]><![endif]-->'."\r\n";
	...
	*/
	public function js_markup()
	{
		...
		// Put together the markup linking to our wixzify-js-files
