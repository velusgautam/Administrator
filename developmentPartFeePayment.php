	...
	<div class="row-fluid">
		<div class="span12">
			...
			<?php
				...
				$dataCheck = $db->query_first("SELECT id, development_fees, payment_status, academic_year FROM `" . TABLE_STUDENT_DEVELOPMENT_FEE . "` Where `student_id`='" . intval(trim($_GET['id'])) . "' AND academic_year = '".$academicYr['academic_year']."' ORDER BY id DESC");
				if ($db->affected_rows > 0)
					if ($dataCheck['academic_year'] == $academicYr['academic_year']) {
						if ($dataCheck['payment_status'] == 0) {
                            ...
                            $balance =$db->query_first("SELECT balance FROM " . TABLE_STUDENT_DEVELOPMENT_FEE_PART . " WHERE development_fee_id = " . $dataCheck['id']." order by part_id desc LIMIT 0, 1");
							if ($partSum['total'] >= $dataCheck['development_fees']) {

								$_SESSION['developmentFeeStatus'] = "Paid Full Development Fees for the Academic Year " . $academicYr['academic_year'] . ".";
			...
			<div class="well no-padding">
				...
				<form id="studentForm" method="post" class="form-horizontal" enctype="multipart/form-data"
				      action="adminBusinessLogic/developmentFeePartPaymentSave.php">
					<?php
						...
						echo "<input type='hidden' value='" . $dataCheck['id'] . "' name='devFeeId'>" . PHP_EOL . PHP_EOL;
						$partSumHidden = (intval($partSum['total']) > 0) ? $partSum['total'] : 0;
						echo "<input type='hidden' value='" . $partSumHidden . "' name='partSum'>" . PHP_EOL . PHP_EOL;
					?>
					...
					<div class="control-group">
						<div class="span2">
							<div class="span12 appln-label-large"><label class="control-label">Payment Done</label></div>
							<?php
								$developmentFees = 0;
						...
						<div class="span3">
							<div class="span4 text-right" style=""><label class="control-label">Bal Devlpmnt Fees:</label></div>
							<div class="span6 text-left">

								<input type="text" style="text-align: right; width: 100px; margin-left: 50px" name="developmentFees"
								       id="developmentFees" tabindex="10" value="<?php print $balance['balance'] ?>" readonly>
								<script type="text/javascript">
						...
						<div class="span3" id="paymentNowDiv">
							...
							<div class="span6 text-left">
								<input type="text" style="text-align: right; width: 100px; margin-left: 0px" name="paymentNow"
								       id="paymentNow" tabindex="13" value="0">
								<script type="text/javascript">
					...
					<div class="form-actions">
						<div class="stdFormAction">
							...
							<input type="submit" id="save" class="btn btn-primary" tabindex="14" value="Pay"/>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#paymentNow').focusout(function () {

			var total = $('#developmentFees').val().trim();
			var paymentNow = $('#paymentNow').val().trim();

//			if(paymentNow > total)
//			{
//				alert("Payment Now Should not be larger than Development Fees");
//				$('#paymentNow').val(0);
//
//					newTotal();
//
//			}
			if (parseInt(paymentNow) < 0) {
				alert("PaymentNow cannot be Negative");
				$('#paymentNow').val(0);
