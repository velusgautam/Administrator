<?php
	...
	if ($_GET['func'] == "streamSelect" && isset($_GET['func'])) {
		if (trim($_GET['schl_id']) != 'All')
			streamSelect(intval($_GET['schl_id']));
		else
		{
			streamSelect('');
	...
	function streamSelect($schl_id) {
		...
		$db->connect();
		if (empty($schl_id)) {
			echo '<select class="input-block-level" data-placeholder="Choose a Stream" tabindex="4" name="stream" style="width: 150px;">
			...
			if ($count === 1) {
				echo '
	                <select class="input-block-level" data-placeholder="Choose a Stream" tabindex="4" name="stream" id="streamSelect" readonly style="width: 150px;">';
	...
	function classSelect($streamVal) {
		...
		if($drop_var[0] == 'All')
		{
			echo '<select class="input-block-level" data-placeholder="Choose a Class"  name="class" id="classSelect" tabindex="5" style="width: 175px;">
		...
		echo "<script type=\"text/javascript\">
					...
					$('#classSelect').change(function(){
				      ...
				      $.get(\"projectinc/excelDD.php\", {
				        ...
				        setTimeout(\"finishAjax_division('result_4', '\"+escape(response)+\"')\", 0);
				      });
				        return false;
					});
				</script>";
...
//**************************************
	if ($_GET['func'] == "division" && isset($_GET['func'])) {
		...
		if (is_numeric($schl_id) && is_numeric($stream_id) && is_numeric($class_id))
			...
			echo '<select class="input-block-level" data-placeholder="Choose a Division"  name="division" id="division" tabindex="6" style="width: 150px;">
	...
	function division($schl_id, $class_id, $stream_id) {
		...
		echo '<select class="input-block-level" data-placeholder="Choose a Division"  name="division" id="division" tabindex="6" style="width: 100px;">';
