<?php
if($_GET['func'] == "cstreamSelect" && isset($_GET['func'])) {
...
function streamSelect($schl_id)
{
    ...
    if(empty($_GET['schl_id']) || trim($_GET['schl_id']) == "All" )
    {
        echo '
   	    ...
   	    <select  data-placeholder="Choose a Stream" tabindex="1" name="stream"  style="width: 200px">
    ...
    else
    {
        ...
        {
            echo '
   	        <select  data-placeholder="Choose a Stream" tabindex="1" name="stream" id="cstreamSelect" readonly style="width: 200px">';
            ...
            $.get(\"projectinc/studentChangeSubDD.php\", {
   		        func: \"classSelect\",
   		        streamVal: $('#cstreamSelect').val()+\"~\"+".$schl_id."
            },
            function(response){
                setTimeout(\"finishAjax_class('cresult_3', '\"+escape(response)+\"')\", 1);
            });
            </script>";
        }
        else
        {
            ...
            echo '
   	            <select  data-placeholder="Choose a Stream" tabindex="2" name="stream" id="cstreamSelect" style="width: 200px">
...
function classSelect($streamVal)
{
    ...
    echo '
   	<select name="classSelect" id="cclassSelect"  data-placeholder="Choose a Class" tabindex="2" style="width: 200px">';
    ...
    echo "<script type=\"text/javascript\">
        $('#cwait_2').hide();
   	    $('#cclassSelect').change(function(){
   	    $('#cwait_3').show();
         $.get(\"projectinc/studentChangeSubDD.php\", {
   		func: \"division\",
   		drop_var: $('#cclassSelect').val()+\"~\"+".$_stream[1]."+\"~\"+".$_stream[0]."
         }, function(response){
           $('#cresult_4').fadeOut();
           setTimeout(\"finishAjax_division('cresult_4', '\"+escape(response)+\"')\", 1);
           $('#wait_3').hide();
         });
       	return false;
   	});
...
function division($classId)
{
    ...
    echo '
   							<select name="division" id="cdivision" style="width: 170px" >';
