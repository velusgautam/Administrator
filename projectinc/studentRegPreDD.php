...
function streamSelect($schl_id)
{
    ...
    {
        $sql_1 = "SELECT DISTINCT  stream_id, IF( stream_id =  '1',  'STATE',  'ICSE' ) AS stream_name FROM `".TABLE_CLASS_MAPPING."` WHERE  schl_id=".$datas[0]."";
					...
					$('#wait_2').show();
					 $('#result_2').show();
					      $.get(\"projectinc/studentRegPreDD.php\", {
							func: \"classSelect\",
							drop_var: $('#streamSelect').val()+\"-\"+".$datas[0]."+\"-\"+".$datas[2]."
					      }, function(response){
    ...
    while($streamSelect = $db->fetch_array( $result ))
    {
	    $sSelected = ($datas[1]==$streamSelect['stream_id'])?" Selected":"";
        echo '<option value="'.$streamSelect['stream_id'].'" '.$sSelected.'>'.$streamSelect['stream_name'].'</option>';
    }
	...
	$.get(\"projectinc/studentRegPreDD.php\", {
		func: \"classSelect\",
		drop_var: ".$datas[1]."+\"-\"+".$datas[0]."+\"-\"+".$datas[2]."
      }, function(response){
...
$db->connect();
	...
	echo '<label class="control-label">Class:</label>
	    ...
	    <option value="All"  selected="selected">Select Class</option>';
		   while($classSelect = mysql_fetch_array( $result )) 
			{
			  $s = ($classSelect['class_id'] == trim($drop_var[2]))?" Selected":"";
			  echo '<option value="'.$classSelect['class_id'].'" '.$s.'>'.$classSelect['class_name'].'</option>';
			}
	echo '</select>
	...
	$.get(\"projectinc/studentRegPreDD.php\", {
		func: \"division\",
		drop_var: ".trim($drop_var[2])."+\"-\"+".$drop_var[0]."+\"-\"+".$drop_var[1]."
      }, function(response){
