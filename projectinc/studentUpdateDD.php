...
function streamSelect($schl_id) {
	...
	<option value="" >Select Stream</option>
        ...
        $sql_1 = "SELECT DISTINCT  stream_id, IF( stream_id =  '1',  'STATE',  'ICSE' ) AS stream_name FROM `"
            . TABLE_CLASS_MAPPING . "` WHERE  schl_id=" . $schl_id . "";
	...
	<select class="input-block-level" data-placeholder="Choose a Stream" tabindex="4" name="stream" id="streamSelect">';
            ...
            while ($streamSelect = $db->fetch_array($result)) {
                echo '<option value="' . $streamSelect['stream_id'] . '" '.($streamSelect['stream_id'] == $_GET['stream_id'])? ' Selected':''.'>' .
                $streamSelect['stream_name'] . '</option>';
            }
...
function classSelect($streamVal) {
    ...
    $db = new Database(DB_SERVER, DB_USER, DB_PASS, DB_DATABASE);
    $db->connect();
    $sql_1 = "SELECT class_id, class_name FROM `" . TABLE_CLASS . "` WHERE  `class_id` IN(Select class_id FROM "
        . TABLE_CLASS_MAPPING . " WHERE `schl_id` =" . $drop_var[1] . " and  stream_id=" . $drop_var[0]
        . ") ORDER BY class_name ASC";

    $result = mysql_query($sql_1);

    echo '<label class="control-label">Class:</label>
							<div class="controls">
...
function division($schl_id, $class_id, $stream_id) {
    ...
    $db->connect();
    $result = mysql_query(
        "SELECT division_id, division_name FROM `" . TABLE_DIVISION . "` WHERE  `division_id` IN ( SELECT division_id FROM "
        . TABLE_CLASS_MAPPING . " WHERE `class_id`=" . $class_id . " AND `stream_id`=" . $stream_id . " AND `schl_id`="
        . $schl_id . " ) ORDER BY division_name ASC "
    ) or die(mysql_error());
