	...
	try {
		...
		if ($_GET["action"] == "list") {
			...
			if (isset($_GET["studid"]) && isset($_GET['ay'])) {
				...
				$sql = "SELECT @a:=@a+1 sl, tsdp.part_id, tsd.id, coalesce(tsd.add_on, 0) as add_on,  tsdp.payment_now, tsdp.balance, tsdp.waive_off, tsd.development_fees as development_fees, tsdp.academic_year,  tsd.student_name, tsd.academic_year, DATE_FORMAT(tsdp.part_date,'%d-%m-%Y') as date,
