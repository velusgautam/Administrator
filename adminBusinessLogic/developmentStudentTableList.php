...
<script type="text/javascript">
    ...
    $(document).ready(function () {
	    ...
	    var stud = <?php print json_encode($devrows); ?>;

	    function getStudentId(code) {
		    if (!!stud && stud != 'null')
	    {
		    return stud.filter(
			    function(stud){return stud.student_id == code}
		    );
	    }
		    else
		    return 0;
        ...
        $('#studentTableContainer').jtable({
            ...
            actions: {
                listAction: 'adminBusinessLogic/developmentStudentListingData.php?action=list&role=<?php echo $_SESSION['Role']; ?>&schlid=<?php echo $_SESSION['SchoolCode'];?>'
            ...
            fields: {
                ...
                fee_payment: {
                    ...
                    display: function (data) {
	                    ...
	                    var output = getStudentId(data.record.student_id);
	                    if(typeof output !== 'undefined' && output.length > 0){

		                    if (output[0].student_id == data.record.student_id){
			                    var $link = $('<button style="margin-left: 5px" class="btn-success">Already Paid</button>');
			                    return $link;
		                    }
	                    }
		                else
		                    {
			                    var $link = $('<a href="#" style="padding-left: 5px"><button class="btn-warning">Pay Development Fees</button></a>');
			                    $link.click(function(){
				                    window.parent.location.href = "developmentFeePayment.php?id=" + data.record.student_id+"&adstatus="+datacode(data.record.admission_status);
			                    });
			                    return $link;
	                    }
